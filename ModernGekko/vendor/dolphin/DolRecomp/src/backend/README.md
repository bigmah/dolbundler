...
The LLVM backend (`llvm/`) and the bytecode backend (`vm/`) start from DolIR
instead of decoded instructions. `vm/` targets hosts that may not create
executable pages at runtime; see `src/vm/README.md`.
